...
public class MyController {
  ...
  /*
   * Given a GET request to the root/path, add the items variable to the scope as
   * “itemList” and render a response using the “index” template.
   */
  @GetMapping("/")
  ...
  @PostMapping("/todos")
  // with @ResponseBody commented out, return a Thymeleaf template (todo.html)
  // @ResponseBody
  public String createTodo(Model model, @ModelAttribute TodoItem newTodo) {
    ...
    items.add(newTodo);
    // return "<li>"+newTodo.getDescription()+"<input type='checkbox'
    // th:checked='${item.isCompleted}' hx-trigger='click'
    // th:hx-post='|/todos/${item.id}/complete|'></li>";
